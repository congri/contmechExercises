...
configuration.
\bi
\item[a)] Find the deformed elements $d\bs{x}^{(1)}$ and $d\bs{x}^{(2)}$ with lengths $ds_1$ and $ds_2$ respectively.
...
\begin{align}
d\bs{x}^{(1)} &= \bs{F}d\bs{X}^{(1)} \\ \nonumber
&= \bmat 1+k & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \emat \frac{dS_1}{\sqrt{2}} \bmat 1 \\1 \\0 \emat =\frac{dS_1}{\sqrt{2}}
\bmat 1+k \\1 \\0 \emat
\end{align}
and:
\be
ds_1 = \left|d\bs{x}^{(1)}\right| = \frac{dS_1}{\sqrt{2}} \sqrt{(1+k)^2+1}
\ee

Similarly:
\begin{align}
d\bs{x}^{(2)} &= \bs{F}d\bs{X}^{(2)} \\ \nonumber
&= \bmat  1+k & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1\emat \frac{dS_1}{\sqrt{2}} \bmat -1 \\1 \\0 \emat =
\frac{dS_1}{\sqrt{2}} \bmat -(1+k) \\1 \\0 \emat
\end{align}
and:
\be
ds_2 = \left|d\bs{x}^{(2)}\right| = \frac{dS_2}{\sqrt{2}} \sqrt{(1+k)^2+1}
\ee
\fi
\item[b)] Evaluate the stretches of these elements $ds_1/dS_1$ and $ds_2/dS_2$ and the change in the angle between them.
...
\begin{align}
\left|\frac{ds_1}{dS_1}\right| &= \sqrt{\frac{(1+k)^2+1}{2}}, \\
\left|\frac{ds_2}{dS_2}\right| &= \sqrt{\frac{(1+k)^2+1}{2}}.
\end{align}
Furthermore the right Cauchy-Green tensor $\bs{C}$ is
\be
\bs{C} = \bmat (1+k)^2 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1\emat
\ee
and:
\begin{align}
ds_1 ds_2 \cos \theta &= d\bs{X}^{(1)}\cdot \bs{C}d\bs{X}^{(2)} \\ \nonumber
&= \frac{dS_1}{\sqrt{2}} \bmat 1 &1 &0 \emat  \bmat (1+k)^2 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1\emat \frac{dS_2}{\sqrt{2}}
\bmat -1 \\1 \\0 \emat \\ \nonumber 
&= \frac{dS_1dS_2}{2}\left(-(1 + k)^2 + 1\right).
\end{align}
\fi
\item[c)] Evaluate part (b) for $k = 1$ and $k = 0.01$.
...
\begin{align}
\left|\frac{ds_1}{dS_1}\right| &= \sqrt{k + 1} = \sqrt{2} \ne 1.581, \\
\left|\frac{ds_2}{dS_2}\right| &= \sqrt{k+1}=\sqrt{2} \ne 1.581
\end{align}
and
\begin{align}
ds_1ds_2\cos \theta &= -k dS_1dS_2,\\
\cos\theta &= -\frac{1}{2}\ne -0.6.
\end{align}
For $k=0.01$
\begin{align}
\left|\frac{ds_1}{dS_1}\right| &= \sqrt{k+1} = 1.00498 \approx 1.005, \\
\left|\frac{ds_2}{dS_2}\right| &= \sqrt{k+1} = 1.00498 \approx 1.005
\end{align}
and
\begin{align}
ds_1ds_2\cos \theta &= -k dS_1dS_2, \\ 
\cos\theta &= -0.0099 \approx -0.00995.
\end{align}
...
\be
\bs{v} = \frac{1}{r^4}\bmat 2x_1x_2 & x_2^2-x_1^2 & 0 \\x_2^2-x_1^2 &-2x_1x_2 & 0\\ 0 & 0 & 0 \emat
\ee
